...
public class noticeWrite extends HttpServlet {
	...
	String usenotice,ntitle,nwriter,ncontent;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(enctypec.equals("application/x-www-form-urlencoded")) {
			...
			this.ncontent = request.getParameter("ncontent");
		}else {
			...
			this.ncontent = URLDecoder.decode(request.getParameter("ncontent"),"UTF-8");
		...
		String[] ndata = {"notice_write",this.usenotice,this.ntitle,this.nwriter,url2,this.ncontent,nindate,viewcount};
		...
		if(result_sign=="ok") {
			pr.print("<script>"
					+ "alert('공지등록이 완료되었습니다.'); location.href='./notice_list.do';"
					+ "</script>");
